...
(`HEAAN2_ROOT` and friends) that is usable only with access to the HEaaN2 source tree; it is inert
for this submission and can be ignored.
...
cache; if you change `HEAAN2_DIR` or the toolchain, `rm -rf submissions/mnist/build` first or the
