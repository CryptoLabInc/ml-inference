...
Layer makeLayer(const LayerGeom &geom, const std::vector<double> &bias,
    ...
    // rotations, each costing a mod-down, against the key-less path's log2(F)
    // free automorphisms. The keys are generated by client_key_generation at
    // this layer's OUTPUT level, because the fold runs after adjust().
