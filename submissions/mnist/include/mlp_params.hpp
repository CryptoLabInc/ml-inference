...
constexpr u32 logRlweDim(const Profile &prof) {
    return prof.ntt_alg == heaan::NTTAlgorithm::CYC_FOR_CI ? prof.log_degree - 1
                                                           : prof.log_degree;
}

constexpr u32 ringDim(const Profile &prof) { return 1U << logRlweDim(prof); }

constexpr u32 slotsPerMsg(const Profile &prof) {
    return 1U << (prof.log_degree - 1);
}

constexpr u32 numBlocks(const Profile &prof, u32 num_images) {
    return (num_images + slotsPerMsg(prof) - 1) / slotsPerMsg(prof);
}

constexpr u32 numCols(const Profile &prof, u32 num_images) {
    return numBlocks(prof, num_images) * ringDim(prof);
}
