...
Layer makeLayer(const LayerGeom &geom, const std::vector<double> &bias,
    ...
    // Keyed fold. Reached whenever the secret key is not lifted, which is the
    // shipped configuration: the cosets {s, 2s, ..., (F-1)s} become explicit
    // rotations, against the key-less path's log2(F) free automorphisms. The
    // keys are generated by client_key_generation at this layer's OUTPUT
    // level, because the fold runs after the ciphertext has been adjusted
    // there.
    if (!lyr.keyless_fold && lyr.fold_factor > 1) {
        ...
        if (lyr.fold_keys.empty())
            throw std::runtime_error(
                ...
                ":1 with a non-invariant key but no fold keys were supplied; "
                "client_key_generation and this stage disagree on "
                "SMALL_LOG_DEGREE");
    }
